...
from vulnstick.tool.grype import Grype, GrypeProfile
...
def test_install_from_path():
    with mock.patch("subprocess.check_call") as check_call, mock.patch(
        ...
        expected_grype_path = f".vulnstick/tools/grype/{normalized_version_str}/{git_describe_val}-{hash_of_git_diff}/local_install"
        tool = Grype.install(
            version=version_str,
            path=".vulnstick/tools/grype/path:_where_grype_is_cloned",
            update_db=False,
        )
