...
def test_effective_year(
    ...
    with patch("vulnstick.utils.grype_db.normalize_to_cve", lambda _: cve_id_lookup):
        assert expected == vuln.effective_year(by_cve=year_from_cve_only)

    # now check that the label entry behaves the same
    with patch("vulnstick.utils.grype_db.normalize_to_cve", lambda _: subject_cve_id):
