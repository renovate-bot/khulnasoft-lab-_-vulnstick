...
function run() {
    tmp_file=$(mktemp /tmp/vulnstick-test.XXXXXX)
    temp_files+=( $tmp_file )
...
run vulnstick result clear

run vulnstick result capture -r test

run vulnstick result list -r test
...
run vulnstick result compare $(vulnstick result list -r test --ids -t grype)
...
run vulnstick label apply $(vulnstick result list -r test --ids -t grype@v0.72.0)

assert_last_output_contains "label: TruePositive"

ID_TO_REMOVE=$(vulnstick label add -i foo -c CVE-1234-ASDF -p test-package -v 1.2.3 -n "testing" --label TP)
run vulnstick label remove ${ID_TO_REMOVE}
