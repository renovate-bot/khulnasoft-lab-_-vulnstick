...
# this is the python package version for vulnstick, based off of the git state
...
check-types: virtual-env-check  ## Run type checks (mypy)
	mypy --config-file ./pyproject.toml src/vulnstick
...
unit: virtual-env-check  ## Run unit tests
	pytest --cov-report html --cov vulnstick -v tests/unit/
