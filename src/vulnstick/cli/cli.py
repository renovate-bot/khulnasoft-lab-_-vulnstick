...
from vulnstick import store
from vulnstick.cli import config, label, result
...
def cli(ctx, verbose: bool, config_path: str):
    ...
    # initialize vulnstick based on the current configuration and
...
@cli.command(name="version", help="show the installed version of vulnstick")
@click.pass_obj
def version(_: config.Application):
    d = pkg_resources.get_distribution("vulnstick")
    if not d:
        raise RuntimeError("vulnstick install information could not be found")
    print(repr(d))
